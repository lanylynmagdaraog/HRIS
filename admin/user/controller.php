			...
			$age = date_diff(date_create($birthdate),date_create('today'))->y;
		 	...
		 	if ($age < 20 ){
		      ...
		      redirect("index.php?view=view&id=".$_SESSION['USERID']);
				...
				$emp->TINNO				= strtoupper($_POST['TINNO']); 
				$emp->SSSNO				= strtoupper($_POST['SSSNO']); 
				$emp->PHNO				= strtoupper($_POST['PHNO']); 
				$emp->EMAILADDRESS		= $_POST['EMAILADDRESS']; 
				$emp->CELLNO			= strtoupper($_POST['CELLNO']);   
				$emp->PAGIBIGNO			= strtoupper($_POST['PAGIBIGNO']);   
				$emp->PERAANO			= strtoupper($_POST['PERAANO']);   
				$emp->GSISNO			= strtoupper($_POST['GSISNO']);   	
...
function doAddCivilService(){  
	if(isset($_POST['save'])){
			...
			$CSE->DATERELEASED = 				$_POST['DATERELEASED'];
			$CSE->create(); 

			message("New CIVIL SERVICE ELIGIBILITY has been saved", "success");
			redirect("index.php?view=view&id=".$_SESSION['USERID']);   
	}
} 
...
function doRemoveCivilService(){   
			...
			message("CIVIL SERVICE ELIGIBILITY has been removed", "success");
			redirect("index.php?view=view&id=".$_SESSION['USERID']);   
} 
